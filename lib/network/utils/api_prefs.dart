...
class ApiPrefs {
  ...
  static const String KEY_HAS_CHECKED_OLD_REMINDERS =
      'has_checked_old_reminders';
  static const String KEY_HAS_MIGRATED_TO_ENCRYPTED_PREFS =
      'has_migrated_to_encrypted_prefs';
  ...
  static Future<void> _migrateToEncryptedPrefs() async {
    ...
    await _prefs?.setStringList(KEY_LOGINS, []);
    await oldPrefs.remove(KEY_LOGINS);

    await _prefs?.setBool(KEY_HAS_MIGRATED, true);
    await oldPrefs.remove(KEY_HAS_MIGRATED);

    await _prefs?.setBool(KEY_HAS_CHECKED_OLD_REMINDERS, true);
    await oldPrefs.remove(KEY_HAS_CHECKED_OLD_REMINDERS);

    await _prefs?.setString(KEY_CURRENT_LOGIN_UUID, '');
  ...
  static Login? getCurrentLogin() {
    ...
    if (_currentLogin == null) {
      ...
      _currentLogin =
          getLogins().firstWhereOrNull((it) => it.uuid == currentLoginUuid);
    }
  ...
  static Future<void> performLogout({bool switchingLogins = false, app}) async {
    ...
    if (!switchingLogins) {
      ...
      final reminderIds =
          reminders?.map((it) => it.id).toList().nonNulls.toList() ?? [];
  ...
  static Tuple2<SchoolDomain, LoginFlow>? getLastAccount() {
    ...
    final loginFlow = _prefs!.containsKey(KEY_LAST_ACCOUNT_LOGIN_FLOW)
        ? LoginFlow.values[lastLogin]
        : LoginFlow.normal;
  ...
  static Future<void> setUser(User user, {app}) async {
    if (isMasquerading()) {
      await updateCurrentLogin((b) => b..masqueradeUser = user.toBuilder(),
          app: app);
    } else {
    ...
    if (userLocale.length == 1) {
      ...
      return Locale.fromSubtags(
        ...
        scriptCode: userLocale[1].length < 5
            ...
            : userLocale[1].substring(
                0,
                min(
                    8,
                    userLocale[1]
                        .length)), // en-unimelb -> en-unimelb (no more than 8)
  ...
  static String? getCurrentLoginUuid() =>
      _getPrefString(KEY_CURRENT_LOGIN_UUID);
  ...
  static bool getHasCheckedOldReminders() =>
      _getPrefBool(KEY_HAS_CHECKED_OLD_REMINDERS) ?? false;

  static Future<void> setHasCheckedOldReminders(bool checked) =>
      _setPrefBool(KEY_HAS_CHECKED_OLD_REMINDERS, checked);

  static int? getCameraCount() => _getPrefInt(KEY_CAMERA_COUNT);

  static Future<void> setCameraCount(int? count) =>
      _setPrefInt(KEY_CAMERA_COUNT, count);
  ...
  static bool? getRatingDontShowAgain() =>
      _getPrefBool(KEY_RATING_DONT_SHOW_AGAIN);
  ...
  static setCurrentStudent(User? currentStudent) {
    ...
    if (currentStudent == null) {
      ...
      _prefs!.setString(
          KEY_CURRENT_STUDENT, json.encode(serialize(currentStudent)));
    }
  }
