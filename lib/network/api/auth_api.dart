...
class AuthApi {
  ...
  Future<CanvasToken?> getTokens(
      MobileVerifyResult? verifyResult, String requestCode) async {
    ...
    return fetch(
        dio.post('${verifyResult?.baseUrl}/login/oauth2/token', data: params));
  }

  Future<MobileVerifyResult?> mobileVerify(String domain,
      {bool forceBetaDomain = false}) async {
    ...
    var mobileVerifyBetaEnabled =
        (forceBetaDomain) && domain.contains('.beta.');
