...
class PandaRouter {
  static WebContentInteractor get _interactor =>
  ...
  static String courseAnnouncementDetails(
          String courseId, String announcementId) =>
  ...
  static String discussionDetails(String courseId, String assignmentId) =>
      assignmentDetails(courseId, assignmentId);
  ...
  static String eventDetails(String courseId, String eventId) =>
      'courses/$courseId/calendar_events/$eventId';

  static String frontPage(String courseId) =>
      '/courses/$courseId/pages/first-page';
  ...
  static String quizAssignmentDetails(String courseId, String quizId) =>
      assignmentDetails(courseId, quizId);
  ...
  static String qrPairing(
      {String? pairingUri, bool isCreatingAccount = false}) {
    if (isCreatingAccount)
      return '$_qrPairing?${_RouterKeys.isCreatingAccount}=${isCreatingAccount}';
  ...
  static String _routerErrorRoute(String url) =>
      '/error?${_RouterKeys.url}=${Uri.encodeQueryComponent(url)}';
  ...
  static String simpleWebViewRoute(
          String url, String infoText, bool limitWebAccess) =>
  ...
  static String submissionWebViewRoute(String url, String title,
          Map<String, String> cookies, bool limitWebAccess) =>
  ...
  static String syllabus(String courseId) =>
      '/courses/$courseId/assignments/syllabus';
  ...
  static void init() {
    if (!_isInitialized) {
      ...
      router.define(syllabus(':${_RouterKeys.courseId}'),
          handler: _syllabusHandler);
      router.define(
          assignmentDetails(
              ':${_RouterKeys.courseId}', ':${_RouterKeys.assignmentId}'),
          handler: _assignmentDetailsHandler);
      router.define(calendar, handler: _calendarHandler);
      router.define(
          courseAnnouncementDetails(
              ':${_RouterKeys.courseId}', ':${_RouterKeys.announcementId}'),
          handler: _courseAnnouncementDetailsHandler);
      router.define(courseDetails(':${_RouterKeys.courseId}'),
          handler: _courseDetailsHandler);
      router.define(courses(), handler: _coursesHandler);
      router.define(
          eventDetails(':${_RouterKeys.courseId}', ':${_RouterKeys.eventId}'),
          handler: _eventDetailsHandler);
      router.define(frontPage(':${_RouterKeys.courseId}'),
          handler: _frontPageHandler);
      router.define(frontPageWiki(':${_RouterKeys.courseId}'),
          handler: _frontPageHandler);
      router.define(gradesPage(':${_RouterKeys.courseId}'),
          handler: _gradesPageHandler);
      router.define(help(), handler: _helpHandler);
      router.define(
          institutionAnnouncementDetails(
              ':${_RouterKeys.accountNotificationId}'),
          handler: _institutionAnnouncementDetailsHandler);
      router.define(legal(), handler: _legalHandler);
      router.define(
          quizAssignmentDetails(
              ':${_RouterKeys.courseId}', ':${_RouterKeys.quizId}'),
          handler: _assignmentDetailsHandler);
  ...
  // Handlers
  static Handler _accountCreationHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    var pairingInfo = QRPairingScanResult.success(
        params[_RouterKeys.pairingCode]![0],
        params[_RouterKeys.domain]![0],
        params[_RouterKeys.accountId]![0]);
  ...
  static Handler _assignmentDetailsHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
  ...
  static Handler _calendarHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    var calendarParams = {
      CalendarScreen.startDateKey: DateTime.tryParse(
              params[_RouterKeys.calendarStart]?.elementAt(0) ?? '') ??
          DateTime.now(),
      CalendarScreen.startViewKey:
          params[_RouterKeys.calendarView]?.elementAt(0) == 'month'
              ? CalendarView.Month
              : CalendarView.Week,
    };
  ...
  static Handler _courseAnnouncementDetailsHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return AnnouncementDetailScreen(params[_RouterKeys.announcementId]![0],
        AnnouncementType.COURSE, params[_RouterKeys.courseId]![0], context);
  });
  ...
  static Handler _domainSearchHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    // Login flow
    String loginFlowString = params[_RouterKeys.loginFlow]?.elementAt(0) ??
        LoginFlow.normal.toString();
    LoginFlow loginFlow =
        LoginFlow.values.firstWhere((e) => e.toString() == loginFlowString);

    return DomainSearchScreen(loginFlow: loginFlow);
  });

  static Handler _eventDetailsHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
  ...
  static Handler _frontPageHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return CourseRoutingShellScreen(
        params[_RouterKeys.courseId]![0], CourseShellType.frontPage);
  });

  static Handler _gradesPageHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return CourseDetailsScreen(params[_RouterKeys.courseId]![0]);
  });
  ...
  static Handler _institutionAnnouncementDetailsHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
  ...
  static Handler _legalHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return LegalScreen();
  });
  ...
  static Handler _loginWebHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    // Auth provider
    String? authProvider =
        params[_RouterKeys.authenticationProvider]?.elementAt(0);
    if ('null' == authProvider) authProvider = null;

    // Login flow
    String? loginFlowString = params[_RouterKeys.loginFlow]?.elementAt(0);
    if (loginFlowString == null || loginFlowString == 'null')
      loginFlowString = LoginFlow.normal.toString();
    LoginFlow loginFlow =
        LoginFlow.values.firstWhere((e) => e.toString() == loginFlowString);
  ...
  static Handler _notParentHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return NotAParentScreen();
  });

  static Handler _qrLoginHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    String? qrLoginUrl = params[_RouterKeys.qrLoginUrl]?.elementAt(0);
    return SplashScreen();
  });

  static Handler _qrTutorialHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return QRLoginTutorialScreen();
  });

  static Handler _qrPairingHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    var pairingInfo = QRUtils.parsePairingInfo(
        params[_RouterKeys.qrPairingInfo]?.elementAt(0));
    var isCreatingAccount =
        params[_RouterKeys.isCreatingAccount]?.elementAt(0) == 'true';
  ...
  static Handler _rootSplashHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return SplashScreen();
  });

  static Handler _routerErrorHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
  ...
  static Handler _simpleWebViewHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    final url = params[_RouterKeys.url]![0];
    ...
    final cookies = (cookiesParam == null || cookiesParam.isEmpty)
        ? {}
        : jsonDecode(cookiesParam);
    final limitWebAccess =
        params[_RouterKeys.limitWebAccess]?.firstOrNull == 'true';
    return SimpleWebViewScreen(url, title, limitWebAccess,
        infoText: infoText == null || infoText == 'null' ? null : infoText,
        initialCookies: cookies);
  });

  static Handler _syllabusHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
    return CourseRoutingShellScreen(
        params[_RouterKeys.courseId]![0], CourseShellType.syllabus);
  });

  static Handler _termsOfUseHandler = Handler(
      handlerFunc: (BuildContext? context, Map<String, List<String>> params) {
  ...
  static Handler _rootWithExternalUrlHandler = Handler(
    ...
    QRPairingInfo? pairingInfo =
        pairingParseResult is QRPairingInfo ? pairingParseResult : null;
    if (pairingInfo != null) {
    ...
    if (urlRouteWrapper.appRouteMatch != null &&
        (ApiPrefs.isLoggedIn() || qrUri != null)) {
    ...
    if (urlRouteWrapper.appRouteMatch != null) {
      ...
      if (limitWebAccess) {
        ...
        locator<QuickNav>().pushRoute(
            context,
            simpleWebViewRoute(
                url, L10n(context).webAccessLimitedMessage, true));
        ...
        locator<FlutterSnackbarVeneer>()
            .showSnackBar(context, L10n(context).routerLaunchErrorMessage);
      }
    }
  }

  static bool get limitWebAccess =>
      ApiPrefs.getUser()?.permissions?.limitParentAppWebAccess ?? false;
  ...
  static _UrlRouteWrapper getRouteWrapper(String link) {
    ...
    final path =
        '/${uri.pathSegments.join('/')}${uri.query.isNotEmpty ? '?${uri.query}' : ''}';
    ...
    return _UrlRouteWrapper(
        path,
        currentDomain == null ? true : currentDomain.contains(uri.host),
        path == '/' ? null : match);
  }
}
...
class _RouterKeys {
  ...
  static final url =
      'url'; // NOTE: This has to match MainActivity.kt in the Android code
}
